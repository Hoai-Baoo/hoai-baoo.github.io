---
title:  "Electronic and Electrical Engineering  - Ho Chi Minh City University of Technology (HCMUT – Bach Khoa)"
date:   2026-03-01 10:00:00 +0700
categories: uni
tags: [hcmut, bachkhoa, engineering, vietnam, university-life]
---

![Trường Đại học Bách Khoa - Đại học Quốc Gia Tp.HCM](/assets/img/bachkhoa.jpg "HCMUT")


## **1. Tổng quan tất cả môn học của khoa Điện**

<!-- ### **1.1. Chuyên ngành** -->
Khoa Điện - Điện tử hiện đang có 4 chuyên ngành chính:
- [**Kỹ thuật Điều khiển và Tự động hóa**](https://drive.google.com/file/d/1_Xw-E7kWiywtT_QqNVaXUTbRJ3CPPHpl/view)
- [**Kỹ thuật Điện tử - Viễn thông**](https://drive.google.com/file/d/1dsR9sO8a-6Qp0ExZvrdhXPcr2AQoVOxo/view)
- [**Kỹ thuật Điện - Điện tử**](https://drive.google.com/file/d/1QFg6WWMk80rm5tJ4el2SBEJuRyz6Uql9/view)
- [**Thiết kế vi mạch**](https://drive.google.com/file/d/1KSHB6gocf-iywE27S4JMQSyzoC9NFlR4/view)

<!-- ### **1.2. Kỹ thuật Điều khiển và Tự động hóa**

Như tên gọi thì ngành này chia ra 2 mảng là **Điều khiển** và **Tự động hoá**. Đặc điểm của ngành này là liên chuyên ngành nên chúng ta cũng cần trang bị kiến thức cơ bản của 1 số ngành khác:
- **Cơ khí**: thiết kế các
- **Điện**: hiểu về điện cao thế, điện ba pha, động cơ điện, biến đổi năng lượng điện ...
- **Điện tử**: thiết kế mạch điện để truyền tín hiệu, đo lường, vi xử lý để điều khiển hệ thống, ...
- **Lập trình nhúng**: code nhúng cho vi điều khiển để xử lý tín hiệu đo lường từ cảm biến, động cơ sang các hệ thống khác, ...
- **Lập trình ứng dụng**: code các ứng dụng bật cao hơn để tương tác với ngừoi dùng như lập trình androids, lập trình web, cloud, ...
- **AI**: xử lý ảnh, hệ thống tự học để tối ưu hoá quy trình, robot, xe tự lái, ... 

Học ngành này thì mình đánh giá khá nặng vì khối lượng kiến thức khổng lồ của nó, nhưng đổi lại bạn sẽ có được cái nhìn tổng quát về hệ thống và nhiều chuyên ngành. Thông thường thì sinh viên tốt nghiệp sẽ làm trong 1 số mảng cụ thể như nhúng, vi mạch, AI, cloud, full-stack, dev-ops, tự động hoá, software dev, và cũng có một số bạn đi du học :D. 

#### **1.2.1. Kỹ thuật Điều khiển**

**Kỹ thuật điều khiển** tập trung vào việc **thiết kế, phân tích và tối ưu hoá hệ thống**.

 Nhìn chung thì ngành này nghiên cứu nhiều hơn vào lý thuyết, áp dụng các mô hình toán học để tính toán tối ưu hoá điều khiển, ước lượng vị trí và đo lường. Kỹ thuật điều khiển sẽ phù hợp cho những bạn đam mê tính toán, thích nghiên cứu tối ưu những mô hình phức tạp.


**Ứng dụng**
- **Manufacturing:** tối ưu hoá quy trình sản xuất
- **Aerospace:** thiết kế hệ thống điều khiển cho tàu vũ trụ
- **Automotive:** điều khiển động cơ xe, tốc độ, phanh, tự hành
- **Energy:** điều khiển tối ưu hoá năng lượng
- **Robotics:** điều khiển chuyển động (motion), di chuyển (movement), navigation(điều hướng), thao tác (manipulation)
- **Autonomous Vehicle**: tàu ngầm, máy bay, xe, tàu tự hành, ...

Hệ thống điều khiển của Trực thăng

<img src="/assets/img/helicopter.png" alt="Helicopter" width="300" height="200">

Autonomous Vehicle

<img src="/assets/img/auv.png" alt="Autonomous Vehicle" width="300" height="200">


#### **1.2.2. Kỹ thuật Tự động hoá**

Kỹ thuật tự động hoá tập trung vào thiết kế, phát triển và thực hiện **tự động hoá hệ thống nhằm giảm thiểu sự can thiệp của con người**. 

Ngược lại với điều khiển, tự động hoá tập trung nhiều hơn về thực hành, ứng dụng thực tế. Ngành kỹ thuật tự động hoá sẽ phù hợp cho những bạn đam mê thực hành, ngành này sẽ cần làm việc trong nhà máy nhiều để lấy kinh nghiệm.


<img src="/assets/img/amazon.png" alt="Hệ thống tự phân loại hàng hoá Amazon width=" width="400" height="200">

Hệ thống lắp ráp ô tô

<img src="/assets/img/robotic.png" alt="Hệ thống lắp ráp ô tô width=" width="400" height="200">

Hệ thống SCADA

<img src="/assets/img/scada.webp" alt="SCADA" width="400" height="200">
 -->

## **2. Các môn học**
### **2.1. Toán và Khoa học tự nhiên (Mathematics and Basic Sciences):** 

- Giải tích 1 
- Giải tích 2
- Đại số tuyến tính
- Xác suất và Thống kê
- Hoá Đại Cương
- Vật Lý 1
- Thí nghiệm vật lý
- Vật lý bán dẫn.
- Phương pháp tính.

### **2.2. Chính trị, Kinh tế, Văn hóa, Xã hội (Socials and Economics):** 

- Pháp luật Việt Nam Đại cương
- Triết học Mác - Lênin
- Kinh tế Chính trị Mác - Lênin
- Chủ nghĩa Xã hội Khoa học 
- Tư tưởng Hồ Chí Minh
- Lịch sử Đảng Cộng sản Việt Nam.

### **2.3. Cơ sở ngành (Core Courses)**:

- Kỹ thuật số
- Trường điện từ (+Vi mạch, ĐTVT, KTĐ )
- Ngôn ngữ lập trình
- Tín hiệu và Hệ thống
- Mạch điện tử
- Giải tích mạch
- Xử lý số Tín hiệu (+Vi mạch, ĐTVT)
- Truyền số liệu và Mạng (+ĐTVT)
- Vi xử lý
- Cơ sở thiết kế vi mạch (+Vi mạch)
- Cơ sở Kỹ thuật điện (+TĐH, KTĐ)
- Cơ sở Điều khiển Tự động (+TĐH, KTĐ)
- Cơ sở Điện tử Công suất (+TĐH, KTĐ)
- Thực tập điện tử 1, 2
- Thực tập điện 1, 2

### **2.4. Chuyên ngành (Specialized Courses):**

-  **Vi mạch**
    - Kiến trúc máy tính
    - Mạch điện tử nâng cao
    - Thiết kế Vi mạch số
    - Kỹ thuật Siêu cao tần
    - Thiết kế vi mạch tương tự

- **Kỹ thuật Điện - Điện tử**
   - Hệ thống Cung cấp Điện
   - Máy điện
   - Hệ thống điện 1, 2
   - Kỹ thuật An toàn điện


- **Kỹ thuật Điện tử - Viễn thông**
    - Thiết kế Hệ thống Nhúng
    - Mạch điện tử Thông tin
    - Kỹ thuật Hệ thống Viễn thông
    - Kỹ thuật Siêu cao tần
    - Thiết kế vi mạch


- **Kỹ thuật Điều khiển và Tự động hoá**
    - Đo lường công nghiệp
    - Lý thuyết Điều khiển Nâng cao
    - Đo lường và Điều khiển bằng Máy tính
    - Thiết bị và Hệ thống Tự động
    - Kỹ thuật Robot


### **2.5. Tốt nghiệp (Graduation Internship/Projects):**

- Thực tập ngoài trường
- Đồ án 1 (chuyên ngành)
- Đồ án 2 (chuyên ngành)
- Đồ án tốt nghiệp (chuyên ngành)

### **2.6. Các môn học khác (Others)**

- **Giáo dục quốc phòng (Military Trainning)**
- **Giáo dục thể chất (Physical education):** Bóng đá, Bóng chuyền, Bóng bàn, Aerobic, Bóng rổ, Cầu lông, Cờ vua, Điền kinh, Bơi, Quần vợt, Bowling, Pickeball.
- **Nhập môn (Introduction to Engineering)**: Nhập môn Về kỹ Thuật
- **Ngoại ngữ (Foreign Languages)**


